...
class PowerGeneratingAssetToyRiskMeasures(RiskMeasureCalculator):
    """Toy model for calculating risk measures for real estate assets."""

    def __init__(self):
        self.model_summary = {"*Toy* model for power plants risk assessment."}
        self.return_period = 100.0  # criteria based on 1 in 100-year flood or cyclone events
        self.measure_thresholds_acute = {
            Threshold.ABS_HIGH: 0.1,  # fraction
            Threshold.ABS_LOW: 0.03,  # fraction
        ...
        self._definition_lookup = {
            RiverineInundation: definition_acute,
            CoastalInundation: definition_acute,
            WaterStress: definition_acute,
            Fire: definition_acute,
            ChronicHeat: definition_cooling,
        }
    ...
    def calc_measure(self, hazard_type: type, base_impact: ImpactDistrib, impact: ImpactDistrib) -> Measure:
        if hazard_type == ChronicHeat:
            return self.calc_measure_cooling(hazard_type, base_impact, impact)
        elif hazard_type in [RiverineInundation, CoastalInundation]:
            return self.calc_measure_acute_rp(hazard_type, base_impact, impact)
        else:
            return self.calc_measure_acute_index(hazard_type, base_impact, impact)

    def calc_measure_acute_rp(self, hazard_type: type, base_impact: ImpactDistrib, impact: ImpactDistrib) -> Measure:
        return_period = 100.0  # criterion based on 1 in 100-year flood events
        histo_loss = base_impact.to_exceedance_curve().get_value(1.0 / return_period)
        future_loss = impact.to_exceedance_curve().get_value(1.0 / return_period)
        loss_change = future_loss - histo_loss
    ...
    def calc_measure_acute_index(self, hazard_type: type, base_impact: ImpactDistrib, impact: ImpactDistrib) -> Measure:
        histo_parameter = base_impact.mean_impact()
        future_parameter= impact.mean_impact()
        parameter_change = (future_parameter - histo_parameter) / histo_parameter

        if (
            future_parameter > self.measure_thresholds_cooling[Threshold.ABS_HIGH]
            and parameter_change > self.measure_thresholds_cooling[Threshold.CHANGE]
        ):
            score = Category.REDFLAG
        elif (
            future_parameter > self.measure_thresholds_cooling[Threshold.ABS_LOW]
            and parameter_change > self.measure_thresholds_cooling[Threshold.CHANGE]
        ):
            score = Category.HIGH
        elif (
            ...
            score = Category.LOW
        return Measure(score=score, measure_0=future_parameter, definition=self.get_definition(hazard_type))
    ...
    def supported_hazards(self) -> Set[type]:
        return set([RiverineInundation, CoastalInundation, WaterStress, Fire])
